...
1. Copy `.env.example` to `.env`. Replace `SESSION_SECRET` with a random value of at least 32 characters. Set `EMAIL_USER` to a Gmail address and `EMAIL_APP_PASSWORD` to a Google App Password. Set `GOOGLE_CLIENT_ID` to a Google Cloud OAuth 2.0 Web client ID.
